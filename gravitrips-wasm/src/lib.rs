use bindings::games::gravitrips::types::Board;
use bindings::games::gravitrips::{ player_a, player_b };
...
const FIRST_PLAYER: bool = true;
const SECOND_PLAYER: bool = false;

mod bindings {
    wit_bindgen::generate!({
        path: "../wit",
        world: "gravitrips-runner",
    });

    use super::GravitripsRunnerComponent;
    export!(GravitripsRunnerComponent);
}

struct GravitripsRunnerComponent;
...
impl bindings::exports::games::gravitrips::runner::Guest for GravitripsRunnerComponent {
    ...
    fn run() -> u64 {
        ...
        let win = loop {
            ...
            let move_result = match to_play {
                true => player_a::make_move(&board),
                false => player_b::make_move(&board),
            };

            if let Err(status) = board.confirm_move(move_result) {
                break GameEnd::Loss(to_play, status);
            } else if board.check_win(move_result) {
        ...
        match win {
            GameEnd::Win(FIRST_PLAYER) | GameEnd::Loss(SECOND_PLAYER, _) => 1,
            GameEnd::Win(SECOND_PLAYER) | GameEnd::Loss(FIRST_PLAYER, _) => 2,
            GameEnd::Draw => 0,
        }
    }
}
